...
public class AmonGrandSlam extends MeleeAttackType {
    ...
    public void update(Attack attack) {
        if (attack.owner.grabbingLedge()) {
            attack.alive = false;
            return;
        }
        if (slamDown) {
            attack.hitbox.height = attack.owner.hitbox.height;
    ...
    public void successfulHit(Attack attack, GameObject hit) {
        if (slamDown) {
            attack.knockback = hit.getCenter().sub(attack.getCenter()).nor().scl(16);
        } else {
